...
public interface OrderRepository extends JpaRepository<OrderEntity, String> {
    ...
    OrderEntity findByIdAndUserEntity(String id, UserEntity userEntity);
    //List<OrderEntity> findByUserEntityAndState(UserEntity userEntity,String state);
    @Query(value = "select * from orders where user_id = :userId and id in (select order_id from order_products where product_id = :productId) and state in ('ADDED','PENDING_PAYMENT')", nativeQuery = true)
    OrderEntity findByProductIdIdAndUseId(@Param("productId") Integer productId, @Param("userId") Integer userId);
    List<OrderEntity> findByUserEntityAndStateOrderByCreatedTimestampDesc(UserEntity userEntity, OrderStatus status);
}
